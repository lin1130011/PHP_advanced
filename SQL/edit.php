...
<body>
    <h1>編輯學員</h1>
    <?php
    $user = $data->query("select * from `students` where `id`='{$_GET['id']}'")->fetch(PDO::FETCH_ASSOC);
    ?>
        ...
        <div>
            ...
            <select name="dept" id="dept">
                ...
                foreach ($dept as $key => $dept_data) {
                    $selected = ($dept_data['id'] == $user['dept']) ? 'selected' : '';
                    echo "<option value='{$dept_data['id']}'$selected>{$dept_data['name']}</option>";
                }
                ?>
            </select>
        </div>
